...
public class Update : ICarterModule
{
    ...
    {
        app.MapPut("/api/v1/contract", 
                async ([AsParameters] Command command, ISender sender) => (await sender.Send(command)).ToOkHttpResult())
            .WithTags("Contracts");
    }

    public record Command(Models.Update.Contract Contract) : IRequest<Result<Models.Get.Contract>>;
    ...
    public class Handler(ContractService service) : IRequestHandler<Command, Result<Models.Get.Contract>>
    {
        public async ValueTask<Result<Models.Get.Contract>> Handle(Command command, CancellationToken cancellationToken)
        {
            ...
            return await service.Update(contract, cancellationToken: cancellationToken);
        }
    }
}