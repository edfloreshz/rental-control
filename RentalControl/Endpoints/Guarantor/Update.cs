...
public class Update : ICarterModule
{
    ...
    {
        app.MapPut("/api/v1/guarantor", 
                async ([AsParameters] Command command, ISender sender) => (await sender.Send(command)).ToOkHttpResult())
            .WithTags("Guarantors");
    }

    public record Command(Models.Update.Guarantor Guarantor) : IRequest<Result<Models.Get.Guarantor>>;
    ...
    public class Handler(GuarantorService service) : IRequestHandler<Command, Result<Models.Get.Guarantor>>
    {
        public async ValueTask<Result<Models.Get.Guarantor>> Handle(Command command, CancellationToken cancellationToken)
        {
            ...
            return await service.Update(guarantor, cancellationToken: cancellationToken);
        }
    }
}